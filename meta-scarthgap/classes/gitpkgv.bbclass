...
#
# Compatibility note (meta-radxa/meta-scarthgap):
# Bitbake 2.18 removed FetchData.revisions in favour of a single
# FetchData.name / FetchData.revision. Scarthgap meta-oe still uses the
# old API, which causes ExpansionError on GITPKGV. This override keeps
# scarthgap's bare-clone (localpath) behaviour while using the new API.
#
# use example:
#
# inherit gitpkgv
